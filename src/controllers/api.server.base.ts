...
import { readOfApiServerBase } from "../model/collectionRead";
...
export const addRoute: AddRoute = ({ LogMiddleware }, globalDataInstance) => {
  ...
  router.get('/api/server/base', LogMiddleware, (request: RequestHaveLogger, response) => {

    readOfApiServerBase(collection).then((result) => {
